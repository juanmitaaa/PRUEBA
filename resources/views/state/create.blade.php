<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight"> {{ __('Add state') }} </h2>
  </x-slot>
  <div class="py-12 max-w-8xl mx-auto sm:px-6 lg:px-8"> 
    ...
    <div class="flex flex-col  border-2 rounded p-3 bg-white"> 
        <div class="font-semibold text-xl tracking-tight border-b">
          <h3>{{__('States')}} <span class="float-right"><a  href="{{ route('dashboard') }}" class="text-indigo-700"> {{ __('Dashboard') }}</a> > <a  href="{{ route('states.index') }}" class="text-indigo-700">{{ __('States') }} </a> > {{ __('Create states') }}</span></h3>
        </div>
      @include('state.form')
