<x-app-layout>
  ...
  <div class="py-12 max-w-8xl mx-auto sm:px-6 lg:px-8"> 
    {!!Form::model($product,['route'=>['products.update',$product],'method'=>'PUT','id'=>'update_product'])!!}
    <div class="flex flex-col  border-2 rounded p-3 bg-white">
       <div class="font-semibold text-xl tracking-tight border-b">
          <h3>{{__('Products')}} <span class="float-right"><a  href="{{ route('dashboard') }}" class="text-indigo-700"> {{ __('Dashboard') }}</a> > <a  href="{{ route('products.index') }}" class="text-indigo-700">{{ __('Products') }} </a> > {{ __('Edit product') }}</span></h3>
        </div> 
