<x-app-layout>
  ...
  <div class="py-12 max-w-8xl mx-auto sm:px-6 lg:px-8"> 
    {!!Form::model($customer,['route'=>['customers.update',$customer],'method'=>'PUT','id'=>'update_customer'])!!}
    <div class="flex flex-col  border-2 rounded p-3 bg-white"> 
      <div class="font-semibold text-xl tracking-tight border-b">
          <h3>{{__('Customers')}} <span class="float-right"><a  href="{{ route('dashboard') }}" class="text-indigo-700"> {{ __('Dashboard') }}</a> > <a  href="{{ route('customers.index') }}" class="text-indigo-700">{{ __('Customers') }} </a> > {{ __('Edit customer') }}</span></h3>
        </div>
