<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight"> {{ __('Create customer') }} </h2>
  </x-slot>
  <div class="py-12 max-w-8xl mx-auto sm:px-6 lg:px-8"> 
    ...
    <div class="flex flex-col  border-2 rounded p-3 bg-white"> 
       <div class="font-semibold text-xl tracking-tight border-b">
          <h3>{{__('Customers')}} <span class="float-right"><a  href="{{ route('dashboard') }}" class="text-indigo-700"> {{ __('Dashboard') }}</a> > <a  href="{{ route('customers.index') }}" class="text-indigo-700">{{ __('Customers') }} </a> > {{ __('Create customer') }}</span></h3>
        </div>
    ...
    <div class="grid grid-cols-12 border-2 mt-4 rounded bg-white p-2">
      ...
      <div class="col-span-3">
        ...
        {!! link_to_route('customers.index', __('Cancel'),null,['class'=>'bg-gray-500 hover:bg-gray-800 text-white font-bold py-2 px-4 rounded inline']) !!} </div>
      </div>
