<x-app-layout>
  <x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight"> {{ __('Create incidence') }} </h2>
  </x-slot>
  <div class="py-12 max-w-8xl mx-auto sm:px-6 lg:px-8"> 
    {!!Form::open(['route'=>'incidences.store', 'method'=>'POST'])!!}
    <div class="flex flex-col  border-2 rounded p-3 bg-white"> 
      <div class="font-semibold text-xl tracking-tight border-b">
        <h3>{{__('Incidences')}} <span class="float-right"><a  href="{{ route('dashboard') }}" class="text-indigo-700"> {{ __('Dashboard') }}</a> > <a  href="{{ route('incidences.index') }}" class="text-indigo-700">{{ __('Incidences') }} </a> > {{ __('Create incidence') }}</span></h3>
      </div>
